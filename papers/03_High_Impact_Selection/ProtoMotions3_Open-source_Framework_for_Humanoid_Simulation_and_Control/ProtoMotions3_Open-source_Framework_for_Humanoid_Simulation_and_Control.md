...
<div class="mermaid">
flowchart TD
  subgraph motion["动作数据"]
    A["AMASS / BONES-SEED 等"]
    B["PyRoki Retargeting<br/>→ G1 / H1 / 自定义 morphology"]
    A --> B
  end
  subgraph sim["多后端仿真"]
    C["Newton · IsaacGym · IsaacLab<br/>Genesis · MuJoCo"]
    D["Sim2Sim 交叉验证"]
    B --> C
    C --> D
  end
  subgraph algo["算法栈"]
    F["PPO / AMP / ASE / MaskedMimic"]
    C --> F
  end
  subgraph dep["部署"]
    G["ONNX 导出 + observation 对齐封装"]
    H["Unitree G1 等实机 / RoboJuDo"]
    F --> G
    G --> H
  end
</div>
