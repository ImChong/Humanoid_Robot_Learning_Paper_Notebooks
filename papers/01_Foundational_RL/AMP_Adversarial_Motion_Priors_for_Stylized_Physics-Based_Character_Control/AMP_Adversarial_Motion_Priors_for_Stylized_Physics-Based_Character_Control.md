...
| **鉴别器 $D$** | 1024 → 512，ReLU | 1D 标量（输入为 $(\Phi(s\_t), \Phi(s\_{t+1}))$） |
...
$$r^G = \exp\left(-0.25 (v^* - v\_{xcom})^2\right)$$
...
$$\min\_D \; \mathbb{E}\_{d^M}\left[(D(\Phi(s), \Phi(s')) - 1)^2\right] + \mathbb{E}\_{d^\pi}\left[(D(\Phi(s), \Phi(s')) + 1)^2\right] + \frac{w\_{GP}}{2} \mathbb{E}\_{d^M}\left[\lVert \nabla D \rVert^2\right]$$
...
$$r^S(s\_t, s\_{t+1}) = \max\left[0, \; 1 - 0.25(D(s\_t, s\_{t+1}) - 1)^2\right]$$
