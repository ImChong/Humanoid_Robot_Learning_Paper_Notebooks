...
其中 $q$ 是关节角度向量，$\ddot{q}$ 是关节角加速度，$\tau$ 是关节扭矩，$M(q)$ 是 $n \times n$ 的**质量矩阵**（也叫惯性矩阵），$n$ 是关节自由度数。
...
- 等效转动惯量 ≈ $m \cdot r^2 + I_{cm} \approx 5 \times 0.2^2 + \frac{1}{12} \times 5 \times 0.4^2 = 0.2 + 0.067 \approx 0.27 \text{ kg·m}^2$

> ⚠️ 附录 F 数值例子中使用 $M = 5 \text{ kg·m}^2$ 是为了计算简洁的简化值，实际值取决于仿真模型的具体参数。
...
| 概念 | 含义 | 单位 |
|------|------|------|
| $M(q)$ 质量矩阵 | 铰接体系统的惯性特性，$n \times n$ 对称正定矩阵 | $\text{kg·m}^2$ |
| $M_{ii}$ 对角元素 | 第 $i$ 个关节的等效转动惯量 | $\text{kg·m}^2$ |
| $M_{ij}$ 非对角元素 | 关节 $i$ 和 $j$ 之间的耦合惯性 | $\text{kg·m}^2$ |
