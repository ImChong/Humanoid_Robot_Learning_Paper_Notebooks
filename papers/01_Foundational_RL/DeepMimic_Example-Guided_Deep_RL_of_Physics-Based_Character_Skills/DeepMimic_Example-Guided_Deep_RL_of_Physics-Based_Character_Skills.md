...
但对于铰接多体系统（如 DeepMimic 的人形角色），运动方程写成**广义坐标**（关节角度）的形式：
...
> 💡 **直觉**：伸直手臂转肩膀 vs 弯曲手臂转肩膀——前者更费力，因为前臂离肩关节远，转动惯量更大。关节构型不同，各关节"感受到"的等效惯性就不同。

#### 膝关节等效质量估算

以 Humanoid 的**右膝关节**（1-DoF 旋转关节）为例，膝关节以下有小腿 + 脚：

- 小腿 + 脚总质量约 5 kg
- 小腿长约 0.4m，质心大约在膝关节下方 0.2m
- 等效转动惯量 ≈ $m \cdot r^2 + I_{cm} \approx 5 \times 0.2^2 + \frac{1}{12} \times 5 \times 0.4^2 = 0.2 + 0.067 \approx 0.27 \text{kg} \cdot \text{m}^2$

> ⚠️ 附录 F 数值例子中使用 $M = 5 \text{kg} \cdot \text{m}^2$ 是为了计算简洁的简化值，实际值取决于仿真模型的具体参数。
...
- **单关节**（1-DoF，如膝关节）： $M$ 退化为标量 → $A$ 也是标量 → 直接除
- **整个角色**（多关节）： $M$ 是 $n \times n$ 矩阵 → $A$ 也是矩阵 → 需要解线性方程组

物理引擎（Bullet）每一步仿真都会根据当前关节角度重新计算 $M(q)$。Stable PD 只需在 $M$ 上加两项，这就是为什么说"几乎无额外开销"。

#### 总结

| 概念 | 含义 | 单位 |
|------|------|------|
| $M(q)$ 质量矩阵 | 铰接体系统的惯性特性， $n \times n$ 对称正定矩阵 | $\text{kg} \cdot \text{m}^2$ |
| $M_{ii}$ 对角元素 | 第 $i$ 个关节的等效转动惯量 | $\text{kg} \cdot \text{m}^2$ |
| $M_{ij}$ 非对角元素 | 关节 $i$ 和 $j$ 之间的耦合惯性 | $\text{kg} \cdot \text{m}^2$ |
