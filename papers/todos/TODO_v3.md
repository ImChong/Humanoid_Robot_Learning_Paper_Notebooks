...
| 8 | ✅ `_data/papers.json` 计数（28）与 `update_badges.py`（29）不一致原因排查 | 修 `prepare_pages.py` 总数聚合加上 subcategories；现 33 == 33 | AI |

### R5 · 第二轮分类铺开（可选，AI 可执行）

| # | 任务 | 验收标准 | 负责人 |
|---|------|---------|--------|
| 9 | ✅ `02_High_Impact_Selection/` 现仅 1 篇，按 progress.json 路线再补 2–3 篇骨架 | 新增 OmniH2O、ExBody2 → 共 3 篇 | AI |
| 10 | ✅ `04_Locomotion/` 现仅 1 篇，再补 2 篇骨架（如 ANYmal Parkour、Extreme Parkour） | 新增 Extreme Parkour、ANYmal Parkour → 共 3 篇 | AI |
...
- **v3 第二轮执行后**（2026-04-19 晚）：33 篇 / 12 完稿 / 17 骨架 + 2 短 stub / 0 空分类（R4#8、R5#9、R5#10 完成）。
